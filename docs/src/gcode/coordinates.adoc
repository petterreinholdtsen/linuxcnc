...
[[cha:coordinate-system]](((Coordinate Systems)))
...
Coordinate system offsets are used to shift the coordinate system from the machine coordinate system.
This allows the G code to be programmed for the part without regard to the part location on the machine.
...
will be applied when an axis is homed.  See <<sec:g92-persistence-cautions,G92 Persistence Cautions>> below.
...
'DISABLE_G92_PERSISTENCE = 1' in the '[RS274NGC]' section of
the '.ini' file.

...
changed LinuxCNC rewrites the pocket number to keep track of where the tools are.
...
after a G41/42 only the last one will move the tool to the compensated position.

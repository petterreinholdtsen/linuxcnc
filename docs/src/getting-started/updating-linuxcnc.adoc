...
=== Setting apt sources[[_setting_apt_sources]]
...
using 'kinstype=both'.
The 'both' setting indicates that both forward and inverse kinematics functions are available and gui
provisions that hide the distinctions of joints and axis letters should not be employed.
For example, for an xyz configuration, specify:
...
provided specific sets of halfiles according to the configuration purpose.
...
. 'ddts' -- differentiator hal components are loaded and connected for each joint (and xy, xyz for trivkins machines)
...
. 'sim_spindle' -- the sim_spindle component is loaded and connected to additional loaded hal components to simulate the inertia of a rotating spindle mass.

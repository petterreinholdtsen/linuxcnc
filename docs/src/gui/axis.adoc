...
AXIS is a graphical front-end for LinuxCNC which features a live preview and backplot.
It is written in Python and uses Tk and OpenGL to display its user interface.
...
Some menu items might be grayed out depending on how you have your .ini file configured. For more information on configuration see the <<cha:ini-configuration,INI Chapter>>.
...
image::images/axis-outofrange.png[align="center","The extents of the program in x axis are shown"]
...
By rotating the mouse wheel, or by dragging with the right mouse button pressed, or by dragging with control and the left mouse button pressed,
...
image::images/axis-emc-status.png[align="center", "LinuxCNC Status Window"]

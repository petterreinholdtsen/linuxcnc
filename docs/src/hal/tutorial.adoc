...
From _fr: Halcmd est un outil en ligne de commande pour manipuler HAL. Il existe
...
zero. footnote:[From _fr: Les champs CodeAddr et Arg ont été utilisés
...
handy multimeter (or analog meter for the old timers). From _fr:  Il permet d'observer les pins,
...
From _fr: Il est possible d'ouvrir et de faire fonctionner simultanément
...
From _fr: Dans cet exemple nous utiliserons le type de contrôle _velocity_ du
composant stepgen.
...
From _fr: Le grand bouton _Canal sélectionné_ en bas, indique que le canal 1 est
...
below the trigger level display. From _fr: Il deviendra alors _descendant_. Notez que la modification
...
image::images/halscope-10.png["Sample Rate Dialog"align="center", align="center"]

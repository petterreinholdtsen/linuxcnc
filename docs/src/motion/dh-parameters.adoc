...
= Setting up "modified" Denavit-Hartenberg (DH) parameters for 'genserkins'
...
The difference between "standard" and "modified" notations is in how the parameters are allocated to the links.
Using the "standard" DH-Parameters in 'genserkins' will *not* give the correct mathematical model.
...
counting starts with 0) while most publications start with the number '1'.
That goes for all the parameters as well.  That is, numbering
starts with a-0, alpha-0, d-0 and ends with a-5, alpha-5 and d-5.  Keep
this in mind when following a publication to set up 'genserkins' parameters.
...
for a Mitsubishi RV-6SDL and how to set the parameters in the HAL file to be used with the 'genserkins' kinematics in LinuxCNC.
The necessary dimensions are best taken from a dimensional drawing provided by the manufacturer of the robot.
